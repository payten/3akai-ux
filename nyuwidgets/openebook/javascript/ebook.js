...
require(
    [
        ...
        "/nyuwidgets/openebook/lib/jquery.ui.slider.js",
        "/nyuwidgets/openebook/lib/BookReader/jquery.colorbox-min.js",
        "/nyuwidgets/openebook/lib/BookReader/jquery.bt.min.js",
        "/nyuwidgets/openebook/lib/BookReader/dragscrollable.js",
        "/nyuwidgets/openebook/lib/BookReader/BookReader.js",
        "/nyuwidgets/openebook/lib/jquery-ui.sortable.js",
        "/nyuwidgets/openebook/lib/jquery.jsonp.js"
    ],
    function($, sakai) {
    /**
     * @name sakai.openebook
     *
     * @class ebook
     *
     * @description
     ...
     */
    sakai_global.openebook = function(tuid, showSettings){

        /////////////////////////////
        // Configuration variables //
        /////////////////////////////

        OPENLIBRARY_SEARCH_URL = "http://archive.org/advancedsearch.php";
        sakai.config.URL.AWDL_OEMBED = "http://dlib.nyu.edu/awdl/services/oembed";
        OPENLIBRARY_SEARCH_PARAMS = {
            "q": "mediatype:texts",
            "fl[]": "identifier,title,mediatype,creator,publisher,date",
            "rows": 10,
            "output": "json"
        };
        ...
        var addBinding = function(){
            ...
            $(ebookSettingsUpdate, rootel).live("click", function(e,ui) {
                ...
                if(settings !== false){
                    ...
                    if (settings.books[identifier]) {
                        settings.books[identifier].caption = formData.caption;
                        settings.books[identifier].reader_start_index = parseInt(formData.reader_start_index);
                        // disable the button
                        $this.addClass("s3d-disabled");
            ...
            $(ebookSettingsRemove, rootel).live("click", function(e,ui) {
                // get the identifier of the book to be removed
                var formData = serializeFormToObject($(this).parents(ebookSettingsForm));

                var dialogHTML = sakai.api.Util.TemplateRenderer(ebookRemoveConfirmationDialogTemplate, {bookId: formData.identifier}, $("#ebook_remove_confirmation_dialog"));
            ...
            $(ebookRemoveConfirm, ebookRemoveConfirmationDialog).live("click", function(e,ui){
                ...
                var bookListItemToRemove = $(ebookListItem, ebookSettingsSelectedBooks, rootel).has("input[name='identifier'][value='"+bookIdToRemove+"']");
            ...
            $(ebookSettingsAdd, rootel).live("click", function(e,ui) {
                ...
                if(settings !== false){                    
                    ...
                    if (settings.books[formData.identifier]) {
                    ...
                    $.jsonp(
                        {
                            url: "http://archive.org/details/"+formData.identifier+"&output=json?callback=?",
                            timeout: 5000,
                            success: function(dataJSON) {
                                bookData = searchData.books[formData.identifier].data;

                                // add book to widget settings
                                settings.books[bookData.identifier] = {data: bookData};
                                settings.books[bookData.identifier].caption = formData.caption;
                                settings.books[bookData.identifier].data = sanitizeBookData(settings.books[bookData.identifier].data);
                                ...
                                settings.order.push(bookData.identifier);
                                settings.order.reverse();
                                ...
                                updateWidgetSettings(settings, function() {
                                    ...
                                    itemEl.find(ebookListItemContent).slideUp(function() {
                                        ...
                                        clonedBookEl.find("textarea[name=caption]").val(settings.books[bookData.identifier].caption);
                                        //add clone to list
                                        ...
                                        if (settings.order.indexOf(bookData.identifier) === 0) {
                                            selectedListEl.prepend(clonedBookEl);
                                        } else {
                                            selectedListEl.find(ebookListItem+":nth-child("+settings.order.indexOf(bookData.identifier)+")").after(clonedBookEl);
                                        }
            ...
            $(ebookShowReader, rootel).live("click", function(e,ui){                                                            
               ...
               if ($(this).parents(ebookSettingsSearchResults).length > 0) { //handle a search result
                   bookData.data = {
                       identifier: $(this).parents(ebookSettingsForm).find("input[name='identifier']").val()
                   };
               } else { // handle a selected book
                   var settings = getSettingsObject();
                   identifier = $(this).parents(ebookPreview).find("input[name='identifier']").val();
                   bookData = settings.books[identifier]                   
               }              
               ...
               $(ebookReaderDialog).jqm({
                ...
                var contentHeight = $(ebookReaderDialog).height() - 60;
                var contentWidth = $(ebookReaderDialog).width();
                ...
                var iframe = $("<iframe>")
                                .attr("src", "http://www.archive.org/stream/"+bookData.data.identifier)
                                .attr("width", contentWidth+"px")
                                .attr("height", contentHeight+"px")
                                .width(contentWidth)
                                .height(contentHeight);
                                
                $(".ebook_reader_dialog_content").html(iframe);
            });
            ...
            $(ebookOrderUpLink + ", " + ebookOrderDownLink, rootel).live("click", function(e) {
              ...
              var identifier = formData.identifier;
              var settings = getSettingsObject();
              var currentIndex = settings.order.indexOf(identifier);
              var newIndex = currentIndex+diff;

              settings.order = $.grep(settings.order, function(val) {return val != identifier;});
              settings.order.splice(newIndex,0,identifier);
            ...
            $(ebookList, ebookSettingsSelectedBooks, rootel).live("sortstop", function(e) {
                ...
                var newOrder = [];
                $(this).find("input[name='identifier']").each(function() {
        ...
        var refreshResultsList = function(listElement, refreshOrder) {
            if (refreshOrder) {
                var settings = getSettingsObject();
                for (var i=0; i<settings.order.length;i++) {
                    var listItemToMove = listElement.find('input[name="identifier"][value="'+settings.order[i]+'"]').parents(ebookListItem);
        ...
        var sanitizeBookData = function(bookData) {
            ...
            bookData.creator = $.map(bookData.creator, function(creator) {
                return escape(creator);
            });
            bookData.publisher = $.map(bookData.publisher, function(publisher) {
    ...
    /**
        ...
        var displayBooks = function() {
            ...
            if (settings.display_style == "listing") {
                ...
                for (var i=0; i<settings.order.length;i++) {
                    ...
                    var iframe = $("<iframe>")
                                    .attr("src", "http://www.archive.org/stream/"+settings.books[settings.order[i]].data.identifier)
                                    .attr("width", contentWidth+"px")
                                    .attr("height", contentHeight+"px")
                                    .width(contentWidth)
                                    .height(contentHeight);
                    $(ebookDisplay, rootel).append(iframe);
                }                
            } else {
        ...
        var refreshBookDataFromAWDL = function(onComplete) {
            ...
            var data = $.extend({}, OPENLIBRARY_SEARCH_PARAMS);
            ...
            $.jsonp({
                url: OPENLIBRARY_SEARCH_URL+"?callback=?",
                data: data,
                timeout: 5000,
                success: function(dataJSON) {
                    ...
                    /*for (var i=0; i<dataJSON.response.docs.length; i++) {
                        widgetData.books[dataJSON.response.docs[i].identifier].data = sanitizeBookData(dataJSON.response.docs[i]);
                    }
        ...
        var performAWDLSearch = function(startIndex) {
            ...
            var data = $.extend({}, OPENLIBRARY_SEARCH_PARAMS);
            data.q = data.q + " AND (title:\""+searchString+"\" OR creator:\""+searchString+"\")";

            // construct query filter for books already selected i.e. fq=-id:{id}
            /*var settings = getSettingsObject();
            if (settings.order && settings.order.length > 0) {
                data.fq += " AND -identifier:(" + settings.order.join(" OR ") + ")";
            }*/
        ...
        var handleAWDLSearch = function(resultsJSON) {
            ...
            if (paginationData.total > 0) {
                ...
                paginationData.rows = OPENLIBRARY_SEARCH_PARAMS.rows;
                ...
                for (var i=0; i<resultsJSON.response.docs.length; i++) {
                    ...
                    if (settings.books.hasOwnProperty(resultsJSON.response.docs[i].identifier)) {
                        dummyWidgetData.books[resultsJSON.response.docs[i].identifier] = settings.books[resultsJSON.response.docs[i].identifier];
                        dummyWidgetData.books[resultsJSON.response.docs[i].identifier].selected = true;
                    } else {
                        dummyWidgetData.books[resultsJSON.response.docs[i].identifier] = {selected: false};
                    }
                    // always refresh the ebook data with the AWDL data
                    dummyWidgetData.books[resultsJSON.response.docs[i].identifier].data = resultsJSON.response.docs[i];
                    dummyWidgetData.order.push(resultsJSON.response.docs[i].identifier);
                }
        ...
        var initBookReader = function(currentBookIdentifier, width, height) {
            
            /*internetArchiveBookReader = new BookReader();
            internetArchiveBookReader.getPageWidth = function(index) {
                return width;
            }
            internetArchiveBookReader.getPageHeight = function(index) {
                return height;
            }
           internetArchiveBookReader.getPageURI = function(index, reduce, rotate) {
               debugger;
                var leafStr = '000';
                var imgStr = (index+1).toString();
                var re = new RegExp("0{"+imgStr.length+"}$");
                var url = 'http://www.archive.org/download/'+currentBookIdentifier+'/img/page'+leafStr.replace(re, imgStr) + '.jpg';
                return url;
            }
            internetArchiveBookReader.getPageSide = function(index) {
                if (0 == (index & 0x1)) {
                    return 'R';
                } else {
                    return 'L';
            ...
            internetArchiveBookReader.getSpreadIndices = function(pindex) {   
            ...
            internetArchiveBookReader.getPageNum = function(index) {
                return index+1;
            }
            internetArchiveBookReader.numLeafs = 15;
            internetArchiveBookReader.bookTitle= 'Intenet Archive Open Library Reader';
            internetArchiveBookReader.bookUrl  = 'http://openlibrary.org';
            internetArchiveBookReader.imagesBaseURL = '../lib/BookReader/images/';
            internetArchiveBookReader.getEmbedCode = function(frameWidth, frameHeight, viewParams) {
    ...
    sakai.api.Widgets.widgetLoader.informOnLoad("openebook");
});
