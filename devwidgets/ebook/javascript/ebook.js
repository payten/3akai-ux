...
require(
    [
        "jquery", "sakai/sakai.api.core", "/dev/lib/jquery/jquery-ui.full.js", 
    ...
    sakai_global.ebook = function(tuid, showSettings){
        ...
        var addBinding = function(){
            ...
            $(ebookSettingsUpdate, rootel).live("click", function(e,ui) {
                ...
                if(settings !== false){
                    ...
                    var nid = parseInt(formData.nid);
                    if (settings.books[nid] != null) {
            ...
            $(ebookSettingsRemove, rootel).live("click", function(e,ui) {
                ...
                var dialogHTML = sakai.api.Util.TemplateRenderer(ebookRemoveConfirmationDialogTemplate, {bookId: formData.nid});
            ...
            $(ebookHideRemoveConfirmation, ebookRemoveConfirmationDialog).live("click", function(e,ui){
                $(ebookRemoveConfirmationDialog).jqmHide().remove();            
            });
            ...
            $(ebookSettingsAdd, rootel).live("click", function(e,ui) {
                ...
                if(settings !== false){                
                    ...
                    if (settings.books[formData.nid] != null) {
                    ...
                    if (settings.order == null) settings.order = [];
            ...
            $(ebookShowReader, rootel).live("click", function(e,ui){
               ...
               var dialogHTML = sakai.api.Util.TemplateRenderer(ebookReaderDialogTemplate, {});
               $(document.body).append(dialogHTML);
               $(ebookReaderDialog).jqm({
                ...
                $(ebookReaderDialog).jqmShow().show();
                ...
                $.jsonp({
                    url: sakai.config.URL.AWDL_OEMBED + "?callback=?",
                    data: oembedParams,
                    timeout: 20000,
                    ...
                    error: function (xOptions, textStatus) {                    
                        ...
                        $(".ebook_reader_dialog_content",ebookReaderDialog).html(errorDiv);
                    }
                });
            });
            ...
            $(ebookSettingsSearchPaginationNext+","+ebookSettingsSearchPaginationPrev, rootel).live("click", function(e,ui){            
                var newStartIndex = parseInt($(this).attr("rel"));
            ...
            $(ebookOrderUpLink + ", " + ebookOrderDownLink, rootel).live("click", function(e) {          
              ...
              var diff = parseInt($this.attr("rel"));
            ...
            $(ebookList, ebookSettingsSelectedBooks, rootel).live("sortstop", function(e) {
                ...
                $(this).find("input[name='nid']").each(function() {
                   newOrder.push(parseInt($(this).val()));
                });
        ...
        var getSortedBooks = function(sortToUse) {
            var settings = getSettingsObject();
            if (settings.order == null || settings.order.length == 0) return [];        
            if (settings.sort == undefined || settings.sort == "custom") return settings.order;

            sortToUse = sortToUse || settings.sort;

            if (sortToUse == "title") {
                var oldOrder = settings.order.slice();            
                var newOrder = oldOrder.sort(function(a,b) {
    ...
    /**
        ...
        var displayBooks = function() {
            ...
            if (settings.display_style == "listing") {
                ...
                if (parseInt(settings.rows) > 0) {
                    var books = $(ebookPreview, ebookDisplay, rootel);
                    if (books.length > parseInt(settings.rows)) {
                        ...
                        var widgetHeight = 0;
                        for (var i=0; i<parseInt(settings.rows);i++) {
        ...
        var refreshBookDataFromAWDL = function(onComplete) {        
            ...
            if (settings.lastSynced && (parseInt((new Date() - new Date(settings.lastSynced))/ 1000 / 60 / 24) <= daysBeforeSyncRequired)) {            
                return onComplete();
            }
        ...
        var performAWDLSearch = function(startIndex) {
            ...
            if (startIndex) data.start = startIndex;
        ...
        var handleAWDLSearch = function(resultsJSON) {
            var paginationData = {
              total: parseInt(resultsJSON.response.numFound)
            };
            if (paginationData.total > 0) {

                paginationData.start = parseInt(resultsJSON.response.start) + 1;
            ...
            sakai.api.Widgets.loadWidgetData(tuid, function(success, data) {
                if (success) {
                    ...
                    refreshBookDataFromAWDL(function() {                   
                        if(show){
                            loadSettings(true);
                            if (widgetData.order == null || widgetData.order.length == 0) {
